...
$paketId = intval($_GET['paket_id'] ?? 0);
    ...
    if (!$paketId || empty($tanggalBerangkat) || empty($penumpang)) {
        ...
        if (!$pkg) {
            ...
            try {
                ...
                setFlash('success', 'Pesanan berhasil dibuat! Silakan lanjutkan ke pembayaran.');
                header('Location: ' . base_url('booking/payment.php?id=' . $bookingId));
                exit;
